...
export default function App() {
  ...
  const handleDrop = async (e, targetFolder) => {
    ...
    if (isFolder === 'folder') {
       ...
       if (updates.some(u => u.old === activeFile)) {
          setActiveFile(updates.find(u => u.old === activeFile).new);
       }
    } else {
       if (updatedFiles.includes(newBasePath)) {
           return logTerm("File already exists in destination.", "error");
       }
    ...
    logTerm(`Committing move to Cloud...`, "success");
    try {
       ...
       logTerm(`Move warning: ${err.message}`, "error");
    }
  };
  ...
  const buildTree = (paths) => {
      ...
      paths.forEach(path => {
         ...
         for (let i = 0; i < parts.length; i++) {
            ...
            if (!node) {
               node = {
                 name: part, path: currentPath, type: isFile && part !== '.keep' ? 'file' : 'folder', children: [],
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        {panels.explorer && (
          <aside className="w-64 flex flex-col" style={{ backgroundColor: 'var(--bg-panel)', borderRight: '1px solid var(--border-color)' }}>
            ...
            <div 
                className="flex-1 overflow-y-auto py-2 list-none"
